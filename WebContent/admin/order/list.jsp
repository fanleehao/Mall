							...
							<c:forEach items="${allOrders}" var="o" varStatus="status">
								<tr onmouseover="this.style.backgroundColor = 'white'"
									...
									<td style="CURSOR: hand; HEIGHT: 22px" align="center"
										width="5%">${o.name }</td>
									<td style="CURSOR: hand; HEIGHT: 22px" align="center"
										width="5%"><c:if test="${o.state ==1 }">1=未付款</c:if> <c:if
											test="${o.state ==2 }">2=发货</c:if> <c:if
											test="${o.state ==3 }">3=已发货</c:if> <c:if
											test="${o.state ==3 }">4=订单完成</c:if></td>
									<td align="center" style="HEIGHT: 22px" width="60%"><input
...
$(function(){
	...
	$(".myClass").click(function(){
		...
		if(txt=="订单详情"){
			...
			var url="/Mall/AdminOrderServlet";
			var obj={"method":"findOrderItemsByOidWithAjax","oid":id};			
			$.post(url,obj,function(data){				
				...
				$.each(data,function(i,obj){
					var td="<tr><td><img src='/Mall/"+obj.product.pimage+"' width='50px'/></td><td>"+obj.product.pname+"</td><td>"+obj.product.shop_price+"</td><td>"+obj.quantity+"</td></tr>";
					$tb.append(td);				
				})
			},"json");
